...
class Agent():
    ...
    def get_action(self, s:int, rand:bool=True) -> int : 
    ...
    def update_qtable(self, trans:Tuple[int, int, int, float]):
        """Agent가 경험한 transition을 사용하여 qtable을 갱신합니다.

        Args:
            trans (Tuple[state, action, next_state, reward])
        """
        s, a, s_prime, r = trans
        a_prime = self.get_action(s_prime)
        
        self.qtable[s, a] = self.qtable[s, a] + self.lr * \
            (r + self.gamma * self.qtable[s_prime, a_prime] - self.qtable[s, a])

        return

    
    def decrease_eps(self):
        """SARSA 학습을 위해 사전 설정한 값을 이용하여 eps-greedy에 사용하는 epsilon값을 변경합니다.
        """
