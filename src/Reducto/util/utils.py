...
def flatten(lists: [[]]) -> []:
    """2차원 리스트를 1차원으로 flatten하는 함수
    Args:
        lists (List[List]): 2차원 리스트
    Returns:
        List: flatten된 1차원 리스트
    """
    return [item for sublist in lists for item in sublist]


def assert_list(items, typ):
    """전달받은 item의 타입/형식이 맞는지 확인하고 틀렸다면 error를 발생시키는 함수
    Args:
        items (Union[Type, List]): 확인하려는 변수
        typ (Type): 변수 자체의 type, 또는 변수(리스트)의 원소들의 type이어야하는 것
    Returns:
        List: 형식이 알맞은 리스트
    """
    ...
    for dp, record in dv_record.items():
        ...
        for i, thresh_i in enumerate(thresh_n):
            if thresh_i:
                thresh += np.linspace(
                equal_space[i],
                equal_space[i+1], int(thresh_i)+2, endpoint=True
            ).tolist()[1:-1]
...
def show_stats(summary, keys, cuts=None, show_mean=True):
    ...
    for key in keys:
        ...
        quantiles = df[key].quantile(cuts).to_list()
