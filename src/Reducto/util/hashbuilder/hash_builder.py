...
class ThreshMap:
    ...
    def get_thresh(self, diff_vector, motion_vector=None):
        """segment에 대한 diff vector를 입력으로 받아서 KNN 모델이 예측한 thresh, 클러스터 중심과의 distance를 반환한다."""
...
class HashBuilder:
    ...
    def generate_threshmap(self,
        ...
        """evaluation result, diff vector, target acc, safe zoon을 바탕으로 threshmap(KNN)을 만들어낸다."""
        ...
        for dp_dv in diff_vectors:
            for dp, dv in dp_dv.items():  # dp: diff method (e.g., edge, pixel, area)
